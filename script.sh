...
curl http://192.168.1.130:4200/ -o /var/www/html/teste 2> /dev/null
...
if [ $mesure -le 120 -a $mesure -gt 10 -a $ent -le 40 -a $ent -ge -25 ]
then
	...
	echo "{\"mesure\":$mesure,\"temp\":$temp,\"pressure\":$pres,\"date\":\"$date\"}" > /var/www/html/last.json
fi
